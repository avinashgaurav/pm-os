...
export function AIAnalysisButton({
  ...
  return (
    <>
      <div className={loading ? 'ai-generating rounded-lg inline-block' : 'inline-block'}>
        <Button
          variant="outline"
          size="sm"
      ...
      {show && (
        <div className="surface hairline rounded-xl p-6 mt-6">
          ...
          {loading ? (
            ...
            <div className="space-y-1">
              {response.split('\n').map((line, i) => {
                ...
                if (line.startsWith('## '))
                  return (
                    <h2 key={i} className="text-sm font-semibold text-primary mt-4 mb-1">
                      {line.slice(3)}
                    </h2>
                  );
                if (line.startsWith('### '))
                ...
                return (
                  <p key={i} className="text-sm text-foreground/90 mb-1 leading-relaxed">
                    {line}
                  </p>
                );
              })}
