...
// Structured document generator — produces professional formatted output
function generateDocument(
  ...
  lines.push(`# ${title}`);
  lines.push(`> ${outputName} | ${date} | Generated by PM OS`);
  lines.push('');
  ...
  for (const section of sections) {
    ...
    for (const line of inputLines) {
      ...
      if (trimmed.startsWith('-') || trimmed.startsWith('•') || trimmed.startsWith('*')) {
        lines.push(trimmed);
      } else if (trimmed.includes(':') && trimmed.indexOf(':') < 35 && trimmed.indexOf(':') > 0) {
  ...
  if (filledSections.length >= 2) {
    ...
    for (const s of filledSections) {
      const firstLine = content[s.key]!.trim().split('\n')[0].slice(0, 100);
      lines.push(`- **${s.label}**: ${firstLine}${content[s.key]!.trim().length > 100 ? '...' : ''}`);
    }
    lines.push('');
  }
  ...
  lines.push('');
  lines.push('- [ ] Review this document with stakeholders');
  lines.push('- [ ] Identify gaps and add missing information');
  lines.push('- [ ] Share with relevant team members');
  lines.push('- [ ] Set follow-up date for review');
  lines.push('');

  lines.push('---');
  lines.push(`*${outputName} — Generated by PM OS*`);
  return lines.join('\n');
}
...
export function DocumentEditor({ category, moduleSlug }: DocumentEditorProps) {
  ...
  // Auto-save draft every 10 seconds while editing
  const triggerAutoSave = () => {
    if (autoSaveTimer) clearTimeout(autoSaveTimer);
    const timer = setTimeout(async () => {
      if (title.trim() && step === 'input') {
  ...
  const validate = (): boolean => {
    ...
    for (const s of activeTemplate.sections) {
      if (s.required && !content[s.key]?.trim()) {
        newErrors[s.key] = `${s.label} is required`;
      }
    }
    setErrors(newErrors);
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    // Simulate processing delay for perceived quality
    await new Promise(r => setTimeout(r, 800));

    const output = generateDocument(title, content, activeTemplate.sections, mod.name, outputName);
    setGeneratedOutput(output);
    setStep('output');
    setGenerating(false);
    toast.success(`${outputName} generated`);
  ...
  const handleExportMd = () => {
    const text = generatedOutput || generateDocument(title, content, activeTemplate.sections, mod.name, outputName);
          ...
          {step === 'list' && (
            <div className="glass-card rounded-2xl flex items-center justify-center min-h-[450px]">
              <div className="text-center p-8 max-w-md">
                ...
                <p className="text-xs text-muted-foreground/70 mb-5">Fill in the structured form, then generate a professionally formatted {outputName.toLowerCase()} ready to share with your team.</p>
          ...
          {step === 'input' && (
            <div className="space-y-5" ref={formRef}>
              <div className="flex items-center justify-between">
                <button onClick={() => {
                  if (title.trim() || filledCount > 0) {
                    if (confirm('You have unsaved changes. Leave anyway?')) setStep('list');
                  } else {
                    setStep('list');
                  }
              ...
              <div id="field-title" className="glass-card rounded-xl p-5">
                ...
                <Input value={title} onChange={(e) => { setTitle(e.target.value); setErrors(prev => ({...prev, title: ''})); triggerAutoSave(); }}
                ...
                <div key={section.key} id={`field-${section.key}`} className="glass-card rounded-xl p-5">
                  ...
                  {section.type === 'text' ? (
                    <Input value={content[section.key] || ''}
                      onChange={(e) => { setContent(prev => ({...prev, [section.key]: e.target.value})); setErrors(prev => ({...prev, [section.key]: ''})); triggerAutoSave(); }}
              ...
              <div className="flex items-center gap-3 justify-between pt-2">
                <p className="text-[10px] text-muted-foreground">Auto-saves every 10 seconds</p>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Button variant="outline" size="sm" onClick={() => setEditingOutput(!editingOutput)} className="gap-1.5">
                    <Pencil className="h-3.5 w-3.5" /> {editingOutput ? 'Preview' : 'Edit Output'}
                  </Button>
              ...
              <div className="glass-card rounded-2xl p-6 min-h-[400px]">
                <div className="flex items-center gap-2 mb-4 pb-3 border-b border-border">
                  ...
                  <span className="text-xs font-semibold uppercase tracking-widest text-primary">Generated {outputName}</span>
                </div>
                ...
                {editingOutput ? (
                  ...
                  <div className="space-y-1">
                    {generatedOutput.split('\n').map((line, i) => {
                      ...
                      if (line.startsWith('- [ ] ')) return <p key={i} className="text-sm pl-3 mb-1 flex items-center gap-2"><span className="w-4 h-4 rounded border border-border inline-block shrink-0" />{line.slice(6)}</p>;
                      if (line.startsWith('- **')) {
