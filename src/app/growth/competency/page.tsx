...
export default function CompetencyPage() {
  ...
  const [currentScores, setCurrentScores] = useState<Record<string, number>>(() => {
    ...
    competencyDimensions.forEach((d) => {
      defaults[d.key] = 3;
    });
    return defaults;
  });
  ...
  return (
    <div>
      <PageHeader
        title="PM Competency Tracker"
        description="Assess and track your PM skill development across key dimensions"
        actions={
          !editing ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart data={latestScores}>
                ...
                <PolarRadiusAxis
                  angle={30}
                  domain={[0, 5]}
                  tick={{ fill: 'oklch(0.4 0.01 270)', fontSize: 10 }}
                />
                <Radar
                  name="Score"
                  dataKey="score"
              ...
              {competencyDimensions.map((d) => (
                <div key={d.key}>
                  ...
                  <Slider
                    ...
                    max={5}
                    step={1}
                    value={[currentScores[d.key]]}
                    onValueChange={(val) => {
                      const v = Array.isArray(val) ? val[0] : val;
                      setCurrentScores((s) => ({ ...s, [d.key]: v }));
                    }}
                  />
                </div>
              ))}
            ...
            <div className="space-y-3">
              ...
              {scores.length === 0 && (
                <p className="text-sm text-muted-foreground text-center py-8">
                  No assessments yet. Click &ldquo;New Assessment&rdquo; to get started.
                </p>
              )}
            </div>
          )}
      ...
      <div className="mt-6">
        <AIAnalysisButton
          ...
          buttonLabel="AI: Dev Plan"
          getData={() => {
            try {
              return latestScores.map((s) => `${s.dimension}: ${s.score}/5`).join('\n');
            } catch {
              return '';
            }
          }}
