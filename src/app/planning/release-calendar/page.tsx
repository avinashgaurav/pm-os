  ...
  return (
    <div>
      <PageHeader
        ...
        description="Manage release timelines and milestones"
        actions={
          <>
            <AIAnalysisButton
              category="planning"
              moduleSlug="release-calendar"
              buttonLabel="AI: Review Timeline"
              getData={() =>
                releases.map((r) => `${r.version} ${r.name} - ${r.date} (${r.status})`).join('\n')
              }
            />
      ...
      {releases.length === 0 ? (
        ...
        <div className="space-y-3">
          {releases.map((r, i) => (
            <div
              key={r.id}
              className="group flex items-center gap-4 surface hairline rounded-xl px-5 py-4"
            >
              <div className="w-16 text-center">
                <p className="text-lg font-bold font-mono text-primary">{r.version}</p>
              </div>
              <div className="h-8 w-px bg-border" />
              <div className="flex-1">
                <p className="text-sm font-medium">{r.name || r.version}</p>
          ...
          <div className="space-y-4">
            ...
            <Select
              ...
              onValueChange={(v: string | null) => {
                if (v) setForm((f) => ({ ...f, status: v as typeof form.status }));
              }}
            >
              <SelectTrigger>
                <SelectValue />
