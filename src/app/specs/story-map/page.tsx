    ...
    {activities.length===0 ? <div className="glass-card rounded-2xl p-12 text-center text-sm text-muted-foreground">Add activities to start mapping user stories</div> : (
      <ScrollArea className="w-full"><div className="flex gap-4 min-w-max pb-4">
        {activities.map(act => {
          ...
          return (
            <div key={act.id} className="glass-card rounded-2xl p-4 w-[240px] shrink-0">
              <div className="flex items-center justify-between mb-3"><h3 className="text-sm font-semibold text-primary">{act.name}</h3><button onClick={()=>removeActivity(act.id)} className="p-0.5 rounded hover:bg-accent"><X className="h-3 w-3 text-muted-foreground" /></button></div>
              <div className="space-y-1.5 mb-3">{act.stories.map((s,i)=>(
                <div key={i} className="group flex items-center gap-2 rounded-md bg-muted/50 px-2.5 py-1.5 text-xs"><span className="flex-1">{s}</span><button onClick={()=>removeStory(act.id,i)} className="opacity-0 group-hover:opacity-100"><X className="h-3 w-3 text-muted-foreground" /></button></div>
              ))}</div>
              <div className="flex gap-1"><Input value={storyInput} onChange={e=>setStoryInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){addStory(act.id,storyInput);setStoryInput('');}}} placeholder="Add story..." className="h-7 text-xs" /><Button size="sm" variant="ghost" onClick={()=>{addStory(act.id,storyInput);setStoryInput('');}} className="h-7 w-7 p-0 shrink-0"><Plus className="h-3 w-3" /></Button></div>
            </div>);
        })}
      </div></ScrollArea>
    )}
